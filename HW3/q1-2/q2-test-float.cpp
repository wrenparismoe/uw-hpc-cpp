...
int main() {
    ...
    std::ofstream kij_performanceCSV("kij-performance-float-O3-march.csv");
    kij_performanceCSV << "n,time,FLOPs" << std::endl;
    std::ofstream jki_performanceCSV("jki-performance-float-O3-march.csv");
    ...
    for (int n=n_min; n <= n_max; n++) {
        ...
        std::vector<float> A = generateRandomMatrix<float>(m, p);
        std::vector<float> B = generateRandomMatrix<float>(p, n);
        std::vector<float> C = generateRandomMatrix<float>(m, n);
